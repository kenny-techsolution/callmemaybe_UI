onMouseoverUnknown : function(chart) {
		var popup = $(".income-chart.chartContainer .unknown_popup");
...
afterRendering: function(chart)
{
	...
	// fix label positioning for longer labels
	$(container).find(".highcharts-axis-labels span").each(function() {
		$(this).css("left", "-4px");
	});

	// add unknown popup and fade it out initially
	$(".income-chart.chartContainer").append("<div class='unknown_popup'><div class='unknown_label'>Unknown Income:</div>" +
	...
	$unknownText.find("tspan").attr("x", 178);
	$unknownText.attr("y", 486);
	...
	// add the chart title
	chart._chart.renderer
	.text("ANNUAL HOUSEHOLD INCOME", 12, 25)
	.attr({
		class: 'title'
	})
	.add();
	...
	var onClick = function(elements) {
		return function() {
			...
			var $unknownStatus = $(".income-chart.chartContainer .unknown_status");
